...
import javax.swing.*;     //Write once, run anywhere                                   // provides interfaces, so that the program runs the same on all platforms
...
public class Snake {
    ...
    private final int SNAKE_LENGTH_DEFAULT = 10;
    ...
    // Others values
    private enum GAME_TYPE {NO_MAZE, BORDER};                 //enum is a special "class" that represents a group of constants (unchangeable variables, like final variables), No_maze etc are the methods used 
    private int selectedSpeed = SNAKE_RUNNING_SPEED_FAST;             // default values when the game will begin (selectedSpeed is the method
    private GAME_TYPE selectedGameType = GAME_TYPE.NO_MAZE;           // default selected game type is a variable where the values will be stored
    private int totalBodyPart;                                       // variable to calculate growth of snake
    private int directionX;                                          // variable for x
    private int directionY;                                         // variable for y
    ...
    public void init() {                        
        ...
        board.setBackground(Color.BLACK);                      // setting Backgroud colour as white
        ...
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);   // method set to close the jFrame when user clicks on exit[x] 
    ...
    public void resetDefaultValues() {
        snakeBodyPart = new JButton[2000];                  //2000 is the max range we set for the number of buttons
    ...
    void startNewGame() {                                    // method for new game
        ...
        board.removeAll();                                  // To clear the exixting elements    (removeAll() is a method)
    ...
    public void createInitSnake() {
        ...
        for (int i = 0; i < totalBodyPart; i++) {
            ...
            snakeBodyPart[i].setBackground(Color.GREEN);                                          // Setting snake body to greenss
    ...
    */
    ...
    void processNextStep() {  //key pressed ke saath combine 
    }
    ...
    public void moveSnakeForward(int headLocX, int headLocY) {   //2nd
    ...
    public void snakeKeyPressed(KeyEvent e) {    //1st
        ...
        if (isRunningDown == true && e.getKeyCode() == 40) {                     //value for down arrow key
         ...
         boolean isBorderTouched = false;                                                      //Border touched is set false as the snake will not hit the border immediately when the game has started
        ...
        if (newHeadLocX == foodLocX && newHeadLocY == foodLocY) {
            ...
            if (score % 50 == 0 && !isBonusFoodAvailable) {                                // whenever the score is a multiple of 50 and bonusfood is on screen currently, then...
